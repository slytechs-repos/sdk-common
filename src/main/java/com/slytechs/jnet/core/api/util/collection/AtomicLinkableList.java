 ...
 * the License.
 */
package com.slytechs.jnet.core.api.util.collection;
...
/**
 * A thread-safe manager of a singly linked list that supports atomic operations
 * for traversal, insertion, deletion, searching, updating, reversal, and
 * index-based operations. The list can use either the element's
 * {@code nextLink} and {@code setNextLink} methods or custom getter and setter
 ...
 */
public final class AtomicLinkableList<T> {

	/**
	 * Creates a new thread-safe {@code AtomicLinkableList} instance with the
	 * specified head element, using the element's {@code nextLink} and
	 * {@code setNextLink} methods for navigation.
	 *
	 * @param <T>  the type of elements in the list
	 * @param head the head element of the list
	 * @return a new {@code AtomicLinkableList} instance
	 */
	public static <T extends Linkable<T>> AtomicLinkableList<T> ofLink(T head) {
	...
	/**
	 * Constructs an empty {@code AtomicLinkableList} with custom getter and setter
	 * functions for managing next references.
	 *
	 * @param getNext the function to get the next element
	 * @param setNext the function to set the next element
	 */
	public AtomicLinkableList(UnaryOperator<T> getNext, BiConsumer<T, T> setNext) {
		this.getNext = getNext;
		this.setNext = setNext;
	}

	/**
	 * Constructs an {@code AtomicLinkableList} with the specified head element and
	 * custom getter and setter functions for managing next references.
	 *
	 * @param head    the head element of the list
	 * @param getNext the function to get the next element
	 * @param setNext the function to set the next element
	 */
	public AtomicLinkableList(T head, UnaryOperator<T> getNext, BiConsumer<T, T> setNext) {
		this(getNext, setNext);
		this.head.set(head);
	}

	/**
	 * Adds an element at the head of the list in a thread-safe manner.
	 *
	...
	public T addFirst(T element) {
		while (true) {
			...
			setNext.accept(element, first);
			if (this.head.compareAndSet(first, element)) {
				return element;
			}
		}
	}
	...
	public void add(int index, T element) {
		...
		while (true) {
			...
			if (prev == null) {
				if (head.compareAndSet(current, element)) {
					return;
				}
			} else {
	...
	public T remove(int index) {
		...
		while (true) {
			...
			if (prev == null) {
				if (head.compareAndSet(current, getNext.apply(current))) {
					return current;
				}
			} else {
	...
	public T insertAfter(T target, T element, Predicate<T> match) {
		while (true) {
			...
			if (prev == null) {
				if (head.compareAndSet(current, target)) {
	...
	public T removeAfter(T target, Predicate<T> match) {
		while (true) {
			...
			if (prev == null) {
				if (head.compareAndSet(current, getNext.apply(next))) {
					return next;
				}
			} else {
	...
	public void update(Predicate<T> match, UnaryOperator<T> updater) {
		...
		while (current != null) {
			if (match.test(current)) {
				...
				if (prev == null) {
					if (head.compareAndSet(current, updated)) {
	...
	/**
	 * Reverses the order of the list in a thread-safe manner.
	 */
	public void reverse() {
		T prev = null;
		T current = head.get();
