...
/**
 * Comprehensive interface for memory abstractions, supporting both single segments and chained structures.
 * 
 * <p>Memory combines all memory management capabilities into a unified interface, providing
 * a complete abstraction for high-performance memory operations. This interface is designed
 * for scenarios requiring full memory control, including content access, bounds management,
 * reference counting, and chain operations.</p>
 * 
 * <h2>Design Philosophy</h2>
 * <p>Memory interface follows a zero-allocation design principle, enabling high-performance
 * operations without garbage collection overhead. Key design elements include:</p>
 * <ul>
 *   <li><strong>Unified Interface:</strong> Combines view, window, and reference capabilities</li>
 *   <li><strong>Chain Support:</strong> Native support for linked memory structures</li>
 *   <li><strong>Pool Integration:</strong> Designed for memory pool compatibility</li>
 *   <li><strong>Reference Safety:</strong> Automatic cleanup via reference counting</li>
 * </ul>
 * 
 * <h2>Common Usage Scenarios</h2>
 * <ul>
 *   <li><strong>Network Packet Processing:</strong> Zero-copy packet parsing and manipulation</li>
 *   <li><strong>Streaming Data:</strong> Efficient buffer management for continuous data flows</li>
 *   <li><strong>Protocol Implementation:</strong> Header parsing with automatic bounds checking</li>
 *   <li><strong>Memory Pooling:</strong> High-performance allocation/deallocation cycles</li>
 * </ul>
 * 
 * <h2>Chain Operations</h2>
 * <p>Memory chains enable handling of fragmented data without copying:</p>
 * <pre>{@code
 ...
 * <p>Implementations should prioritize:</p>
 * <ul>
...
public interface Memory extends MemoryView, MemoryWindow, MemoryRef {
     ...
     * 
     * <p>This convenience method provides consistent null checking logic across
     * the memory API, testing both for Java null references and native NULL pointers
     * (address = 0). This is essential when working with native memory or
     * memory-mapped regions.</p>
     * 
    ...
    /**
     * Creates a Memory wrapper for the specified MemorySegment starting at the given offset.
     * 
     * <p>This factory method creates a Memory view that spans from the specified offset
     * to the end of the MemorySegment. The resulting Memory object provides full
     * access to the segment's capabilities through the Memory interface.</p>
     * 
     ...
     * 
     * <p>This factory method creates a Memory view with explicit bounds, enabling
     * precise control over the accessible memory region. The capacity parameter
     * defines the size of the accessible region starting from the offset.</p>
     * 
     * @param segment the backing MemorySegment
     * @param offset the starting offset within the segment
     ...
     * this method considers the entire chain structure.</p>
     * 
     * <p><strong>Performance Note:</strong> This operation requires chain traversal
     * and has O(n) complexity where n is the number of segments in the chain.
     * For frequently accessed values, consider caching the result.</p>
     * 
     ...
     * of each segment to provide the total amount of usable data. This is the
     * primary method for determining the addressable range when using chain-relative
     * offsets in methods like {@link #asMemorySegmentAt(long)}.</p>
     * 
     * <p><strong>Data vs Capacity:</strong> While {@link #chainCapacity()} returns
     * total allocated space, this method returns only the currently active data
     * regions, which may be smaller due to data bounds management.</p>
     * 
     ...
     * 
     * <p>This method counts the total number of linked Memory segments, providing
     * useful information for debugging, performance analysis, and memory allocation
     * strategies. For single (non-chained) memory objects, this returns 1.</p>
     * 
     * <p><strong>Performance Note:</strong> This operation requires chain traversal
     * and has O(n) complexity. The default implementation returns 1, which is
     * correct for non-chained memory implementations.</p>
     * 
     ...
     * memory as this Memory object.</p>
     * 
     * <p>The default implementation creates a slice of the underlying MemorySegment
     * covering the data region. Implementations may override this for custom
     * ByteBuffer creation logic.</p>
     * 
     ...
     * 
     * <p>The default implementation handles both single and chained memory cases,
     * using {@link #seekMemory(long)} for navigation in chained structures.</p>
     * 
     ...
     * 
     * <p>Uses the static {@link #isNull(MemorySegment)} method to check the
     * underlying MemorySegment for null conditions.</p>
     * 
     ...
     * 
     * <p>The default implementation calculates capacity as the difference between
     * memory end and offset: {@code memoryEnd() - memoryOffset()}.</p>
     * 
