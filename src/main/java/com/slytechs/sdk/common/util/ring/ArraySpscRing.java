...
/**
 * Single-producer, single-consumer bounded ring backed by a power-of-two
 * sized array.
 *
 * <p>
 * Lock-free under the SPSC contract. Producer-side and consumer-side cursors
 * advance independently using {@code VarHandle} acquire/release semantics.
 * No atomic compare-and-swap is required because each side has exactly one
 * thread.
 * </p>
 *
 * <p>
 * The capacity supplied to the constructor is rounded up to the next power
 * of two for fast index masking. Use {@link #capacity()} to read the actual
 * capacity after construction.
 * </p>
 *
 * <p>
 * <b>Thread safety:</b> exactly one thread may call producer-side methods
 * ({@link #offer}, {@link #put}, {@link #tryOffer}) and exactly one thread
 * may call consumer-side methods ({@link #poll}, {@link #take},
 * {@link #tryPoll}). The producer and consumer threads may be different.
 * </p>
 *
 ...
 */
public final class ArraySpscRing<T> implements BoundedRing<T>, SpscCapable {
	...
	private static VarHandle findHandle(String name) {
		try {
			return MethodHandles.lookup().findVarHandle(ArraySpscRing.class, name, long.class);
	...
	private long producerSeq;

	@SuppressWarnings("unused")
	private long consumerSeq;

	public ArraySpscRing(int requestedCapacity) {
	...
	public boolean offer(T item) {
		...
		long producer = (long) PRODUCER.getOpaque(this);
	...
	public T poll() {
		...
		// clear the slot to release object reference
