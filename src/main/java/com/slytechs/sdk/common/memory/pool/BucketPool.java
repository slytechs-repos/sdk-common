 ...
 * <pre>{@code
 * long[] sizes = {1518, 9000, 16384, 65536};
...
public class BucketPool<T extends Poolable> implements Pool<T> {
    ...
    /**
     * Factory for creating poolable objects in a bucketed pool.
     * Receives both allocator and bucket size.
     */
    @FunctionalInterface
    public interface BucketFactory<T extends Poolable> {
        T create(SegmentAllocator allocator, long bucketSize);
    }
    ...
    public BucketPool(PoolSettings settings, long[] sizes, BucketFactory<T> factory) {
        ...
        for (int i = 0; i < sizes.length; i++) {
            ...
            buckets[i] = new FreeListPool<>(bucketSettings, 
                    allocator -> factory.create(allocator, bucketSize));
        }
    }
