 ...
 * 
 * Tests the view architecture with direct and mapped binding modes, reference
...
class BoundViewTest {
	...
	class ViewStateTests {
		...
		void testRebinding() {
		    ...
		    // In new model, views don't affect refcount
		    assertEquals(firstRefCount, baseMemory.refCount()); // Should stay the same
		    assertTrue(boundView.isBound());
	...
	class ReferenceCountingTests {
		...
		void testRebindingRefCount() {
		    ...
		    // Get initial refcounts
		    int mem1InitialRef = baseMemory.refCount();
		    int mem2InitialRef = memory2.refCount();
		    
		    boundView.bind(baseMemory);
		    
		    // Views don't affect refcount in new model
		    assertEquals(mem1InitialRef, baseMemory.refCount());

		    // Rebind to different memory
		    boundView.bind(memory2);

		    // Both memories should have unchanged refcounts
		    assertEquals(mem1InitialRef, baseMemory.refCount());
		    assertEquals(mem2InitialRef, memory2.refCount());
		}
