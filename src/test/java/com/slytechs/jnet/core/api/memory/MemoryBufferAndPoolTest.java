...
class MemoryBufferAndPoolTest {
    ...
    @Test
    void testMemoryBufferMutableOffsets() {
        try (Arena arena = Arena.ofConfined()) {
            MemorySegment segment = arena.allocate(100);
            MemoryBuffer buffer = new MemoryBuffer(Memory.of(segment, 0));
    ...
    void testMemoryPoolAllocationAndRelease() {
        ...
        MemoryBuffer buf3 = pool.allocate();
        ...
        MemoryBuffer buf1Realloc = pool.allocate();
    ...
    @Test
    void testMemoryBufferResetForReuse() {
        MemoryPool<MemoryBuffer> pool = createPool();
        MemoryBuffer buffer = pool.allocate();
        ...
        buffer.decrementRef();
        MemoryBuffer reused = pool.allocate();
